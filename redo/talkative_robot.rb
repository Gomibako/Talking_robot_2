...
current_user = User.new(name: User.get_user_name, 
                         age: User.get_user_age, 
                      gender: User.get_user_gender,
                   spaghetti: User.get_user_spaghetti)


puts "Questionaire by #{author.name}"

number_of_linefeeds(2)

grocery_list = GroceryList.new("grocery_list.txt", current_user)

puts  "This is #{grocery_list.owner.name}'s Grocery List:"
puts grocery_list
